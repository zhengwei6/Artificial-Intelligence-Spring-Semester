...
'''
    ...
    return 函數是否執行成功
'''
def SendStep(id_package, listStep):
    global socketServer
    ...
    cntStep = len(listStep)
    if(cntStep > 100):
        print("[Error] : step too long(must less than 100)")
        cntStep = 100
    rbHeader = structItem.pack(cntStep, id_package)

    for i in range(cntStep):
        rbHeader += structItem.pack(listStep[i][0], listStep[i][1])
