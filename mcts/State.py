...
class State(object):
    def __init__(self, state=None, board=None, playerNo=0, visitCount=0, winScore=0):
        self.state = state
        self.board = board
        ...
        """
        GitHub UCT 算法
        UCT = (nodeWinScore / (double) nodeVisit) + 1.41 * Math.sqrt(Math.log(totalVisit) / (double) nodeVisit)
        """
